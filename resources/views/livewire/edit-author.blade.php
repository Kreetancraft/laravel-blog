<div class="space-y-6">
    <div class="space-y-3">
        ...
        <div>
            ...
            <flux:subheading>{{ __('Edit author.') }}</flux:subheading>
        </div>
    </div>
    ...
    <form wire:submit="save" novalidate>
        <flux:card class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-2 border-t border-zinc-200 pt-5 dark:border-zinc-700">
                <flux:button type="submit" variant="primary" icon="check" wire:loading.attr="disabled" wire:target="save" data-test="save-author" :disabled="$errors->any()">{{ __('Save') }}</flux:button>
            </div>
        </flux:card>
    </form>
